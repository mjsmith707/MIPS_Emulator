...
void MIPS_MFHI() {
    ...
    memory->storeWordPhys(0xA0000000, 0x00004010);   // mfhi $t0
    cpu0->stepCPU(1);
...
void MIPS_MFLO() {
    ...
    cpu0->setRegister(8, 0x0);
    memory->storeWordPhys(0xA0000000, 0x00004012);   // mflo $t0
    cpu0->stepCPU(1);
    ASSERT_EQUAL(0xdeadbeefu, cpu0->getRegister(8));
}
...
void MIPS_MOVN() {
    ...
    memory->storeWordPhys(0xA0000000, 0x0109580b);   // movn $t3, $t0, $t1   true
    memory->storeWordPhys(0xA0000004, 0x010a600b);   // movn $t4, $t0, $t2   false
    cpu0->stepCPU(2);
...
void MIPS_MOVZ() {
    ...
    cpu0->setRegister(11, 0x0);         // t3 = 0
    cpu0->setRegister(12, 0x0);         // t4 = 0
    memory->storeWordPhys(0xA0000000, 0x0109580a);   // movn $t3, $t0, $t1   false
    memory->storeWordPhys(0xA0000004, 0x010a600a);   // movn $t4, $t0, $t2   true
    cpu0->stepCPU(2);
...
void MIPS_MTHI() {
    ...
    cpu0->setRegister(8, 0xdeadbeef);   // t0 = 0xdeadbeef
    memory->storeWordPhys(0xA0000000, 0x01000011);   // mthi $t0
    cpu0->stepCPU(1);
...
void MIPS_MTLO() {
    ...
    cpu0->setRegister(8, 0xdeadbeef);   // t0 = 0xdeadbeef
    memory->storeWordPhys(0xA0000000, 0x01000013);   // mtlo $t0
    cpu0->stepCPU(1);
